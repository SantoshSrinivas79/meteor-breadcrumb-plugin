...
var enrichRouteObject = function(route, params, queryParams, isCurrent) {
...
var repairStack = function() {
  ...
  console.log("stack",currentStack[0]);
  ...
  for(index = 0; index < currentStack.length && index < Breadcrumb._stack.length; ++index) {
    ...
    if(curr.routeName != prev.routeName || !_.isEqualPicked(curr.params, prev.params, getParams(curr.route.path)) || ! _.isEqual(curr.queryParams, prev.queryParams)) {
      console.log("Breadcrumb.Mismatch", curr, prev);
      break;
    }
  }
  ...
  for(; index < currentStack.length; ++index) {
    ...
    console.log("Breadcrumb.Evaluate", currSpec);
    Breadcrumb._stack[index] = enrichRouteObject(currSpec.route, currSpec.params, currSpec.queryParams, index == currentStack.length-1);
  }
...
UI.registerHelper('Breadcrumb', function(template) {
  return _.reject(Breadcrumb.getStack(), function(entry) { return entry.title == null; });
});
UI.registerHelper('BreadcrumbNonEmpty', function(template) {
  return _.some(Breadcrumb.getStack(), function(entry) { return entry.title != null; });
});
